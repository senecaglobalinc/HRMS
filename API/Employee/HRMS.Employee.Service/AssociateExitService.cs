...
namespace HRMS.Employee.Service
{
    ...
    /// </summary>
    public class AssociateExitService : IAssociateExitService
    {
        ...
        #region Constructor
        public AssociateExitService(EmployeeDBContext employeeDBContext,
        ...
        public async Task<ServiceResponse<AssociateExitRequest>> GetByEmployeeId(int employeeId, bool isDecryptReq = false)
        {
            ...
            try
            {
                ...
                var (programManagerId, reportingManagerId, projectId,leadId, isSuccessApiCall, responseMessage) = await GetManagersInfo(associate?.FirstOrDefault()).ConfigureAwait(false);
                if (!isSuccessApiCall)
                {
                    response.IsSuccessful = false;
                ...
                if (reportingManagerId != 0)
                    reportingManager = await m_EmployeeContext.Employees.Where(pa => pa.IsActive == true && pa.EmployeeId == reportingManagerId)
                                       .Select(c => c.FirstName + " " + c.LastName).FirstOrDefaultAsync();

                if (programManagerId != 0)
                    programManager = await m_EmployeeContext.Employees.Where(pa => pa.IsActive == true && pa.EmployeeId == programManagerId)
                                     .Select(c => c.FirstName + " " + c.LastName).FirstOrDefaultAsync();
                if(leadId != 0)
                {
                    lead = await m_EmployeeContext.Employees.Where(pa => pa.IsActive == true && pa.EmployeeId == leadId)
                                    .Select(c => c.FirstName + " " + c.LastName).FirstOrDefaultAsync();
                } 

                ServiceResponse<Project> projectDetails = new ServiceResponse<Project>();

                if (projectId != 0)
                {
                    projectDetails = await m_ProjectService.GetProjectByID(projectId);
                }
                else
                {
                    projectDetails.Item = new Project()
                    {
                        ProjectId = 0,
                        ProjectName = null
                    };
                ...
                if (associateExit != null && associateExit.Count > 0)
                {
                    ...
                    associateExitRequest = (from pa in associate
                                            ...
                                            select new AssociateExitRequest
                                            {
                                                ...
                                                ProjectId = projectDetails.Item.ProjectId,
                                                ProjectName = projectDetails.Item.ProjectName,
        ...
        public async Task<ServiceListResponse<ExitDashboardResponse>> GetAssociatesForExitDashboard(string userRole, int employeeId, string dashboard, int departmentId)
        {
            ...
            if (Roles.Associate.GetEnumDescription().TrimLowerCase().Equals(userRole.ToLower().Trim()))
            {
                ...
                {
                    associateAllocation = await m_ProjectService.GetAssociateAllocationsByEmployeeId(employeeId);
            ...
            else if (Roles.TeamLead.GetEnumDescription().TrimLowerCase().Equals(userRole.ToLower().Trim()) && dashboard.ToLower() == StringConstants.DeliveryDashboard.ToLower())
            {
                ...
                associateAllocation = await m_ProjectService.GetAssociateAllocationsByLeadId(employeeId);
                // filtering associates for primary project TL's till multiple project allocation flow is finalized.
                // once multiple project allocations flow is defined then filtering out for IsPrimary column need to be removed
                allocationDtls = associateAllocation?.Items?.Where(x => x.IsPrimary == true).Select(x => x).ToList();
        ...
        public async Task<ServiceResponse<int>> RequestForWithdrawResignation(int employeeId, string resignationRecommendation)
        {
            ...
            try
            {
                ...
                if (resignationRecommendation != null)
                {
                    ...
                    var (programManagerId, reportingManagerId, projectId,leadId, isSuccessApiCall, responseMessage) = await GetManagersInfo(employee).ConfigureAwait(false);
                    if (isSuccessApiCall == false)
                    {
                        response.IsSuccessful = false;
                        response.Message = responseMessage;
                        return response;
        ...
        public async Task<ServiceResponse<int>> AssociateExitSendNotification(int employeeId, int notificationType, int? departmentId, int? projectId, [Optional] string resignationRecommendation, [Optional] string exitFeedback)
        {
            ...
            List<string> subDirectories_AssociateExit_GenericMail = new List<string> { "NotificationTemplate", "AssociateExit_GenericMail.html" };
            ...
            try
            {
                ...
                {
                    allocation = await m_ProjectService.GetAssociateAllocationsByEmployeeId(employeeId);
                ...
                templateFilePath = "\\NotificationTemplate\\AssociateExit_ResignationStatus.html";
                ...
                //if (empDepartment == "Delivery")
                //{
                //templateBody = templateBody.Replace("{ProjectNameLable}", "<th><strong>Project</strong></th>");
                templateBody = projectName != "" ? templateBody.Replace("{ProjectName}", "<td>" + projectName + "</td>") : templateBody.Replace("{ProjectName}", "<td>NA</td>");

                //templateBody = templateBody.Replace("{CriticalLable}", "<th><strong>Critical</strong></th>");
                templateBody = critical != "" ? templateBody.Replace("{Critical}", "<td>" + critical + "</td>") : templateBody.Replace("{Critical}", "<td>NA</td>");

                //templateBody = templateBody.Replace("{ClientNameLable}", "<th><strong>Client Name</strong></th>");
                ...
                {
                    templateFilePath = Utility.GetNotificationTemplatePath(currentDirectory,subDirectories_AssociateExit_GenericMail);                    
                    templateBody = GetNotificationTemplate(templateFilePath);
                ...
                if (notificationType == Convert.ToInt32(NotificationType.ActivitiesInProgress))
                {
                    ...
                    var financeTemplatePath = "\\NotificationTemplate\\ITReturnsNotification.html";
                    ...
                    foreach (string file in Directory.EnumerateFiles(Environment.CurrentDirectory + "\\NotificationTemplate\\Finance"))
                    {
                ...
                if (notificationType == Convert.ToInt32(NotificationType.ExitInterviewCompleted))
                {
                    ...
                    {
                        string secondTemplatePath = Utility.GetNotificationTemplatePath(currentDirectory,subDirectories_AssociateExit_GenericMail);
                ...
                {
                    templateFilePath = "\\NotificationTemplate\\AssociateExit_ResignationStatusWithReason.html";
                    templateBody = GetNotificationTemplate(templateFilePath);
        ...
        private async Task<(int programManagerId, int reportingManagerId, int projectId, int leadId, bool isSuccessApiCall, string responseMessage)> GetManagersInfo(Entities.Employee employeeInfo)
        {
            var isDeliveryDepartment = false;
            var departments = await m_OrgService.GetAllDepartments();
            int? programManagerId = null;
            int? reportingManagerId = null;
            int? projectId = null;
            int? leadId = null;
            if (departments.IsSuccessful && employeeInfo != null)
            {
                var deliveryDepartmentIds = departments.Items.Where(x => x.DepartmentCode.ToLowerInvariant() == "delivery").Select(y => y.DepartmentId).ToArray();

                if ((bool)(deliveryDepartmentIds?.Any(x => x == employeeInfo.DepartmentId.Value)))
                {
                    isDeliveryDepartment = true;
                }

                ServiceListResponse<AssociateAllocation> allocation = await m_ProjectService.GetAssociateAllocationsByEmployeeId(employeeInfo.EmployeeId);

                var associateAllocation = allocation.Items?.Where(st => st.IsPrimary == true).FirstOrDefault();

                if (isDeliveryDepartment && associateAllocation != null)
                {
                    var projectManagersList = await m_ProjectService.GetActiveProjectManagers();
                    var projectManagerDetails = projectManagersList.Items.Where(x => x.ProjectId == associateAllocation.ProjectId).SingleOrDefault();

                    programManagerId = projectManagerDetails?.ProgramManagerId;
                    reportingManagerId = projectManagerDetails?.ReportingManagerId;
                    leadId = associateAllocation?.LeadId;
                    projectId = associateAllocation.ProjectId;
                }
                ...
                if (associateAllocation == null)
                {
                    ...
                    programManagerId = employeeInfo.ReportingManager;
                    reportingManagerId = employeeInfo.ReportingManager;
                }
            }

            return (programManagerId ?? 0, reportingManagerId ?? 0, projectId ?? 0, leadId??0, departments.IsSuccessful, departments.Message);
        }
