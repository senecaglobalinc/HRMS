...
export class NotificationTypeService {
  ...
  GetNotificationTypes() {
    this._httpclient
      .get(this._serverURL + this._resources.GetNotificationType)
  ...
  public AddNotificationType(createObj: NotificationType) {
    ...
    if (this.editMode == false) return this._httpclient.post(_url, createObj);
  }

  public UpdateNotificationType(createObj: NotificationType) {
    let _url = this._serverURL + this._resources.UpdateNotificationType;
    if (this.editMode == true) return this._httpclient.post(_url, createObj);
  }

  public DeleteNotificationType(
    notificationTypeId: number,
    categoryId: number
  ) {
    let _url =
      this._serverURL +
      this._resources.DeleteNotificationType +
      notificationTypeId +
      '&categoryId=' +
      categoryId;
