...
import { SkillsData } from '../../master-layout/models/project-role.model';
...
export class DashboardService {
  ...
  getDashboardNotificationCount() {
    ...
    switch (this._roleName) {
      ...
      case 'HRA': {
        _url +=
          '/SystemNotification/GetHRANotificationCount?notificationTypeId=2';
        break;
      }
      case 'SystemAdmin': {
        _url += '/SystemNotification/CountAdminNotifications';
  ...
  getHRANotifications() {
    let _url = this._employeeServerURL + this._resources.getRejectedProfiles;;

    return this.httpClient.get(_url);
  }
  getAssociateKRAs(userName: string) {
    let _url =
      this._serverURL + '/KRA/GetAssociateKRADetails?emailId=' + userName;
  ...
  updateItStatus(empCode: string, taskId: string) {
    let _url =
      this._serverURL +
      '/SystemNotification/UpdateItNotification?employeeCode=' +
      empCode +
      '&taskId=' +
      taskId;
    return this.httpClient.get(_url);
  ...
  updateFinanceStatus(empCode: string, taskId: string) {
    let _url =
      this._serverURL +
      '/SystemNotification/UpdateFinanceNotification?employeeCode=' +
      empCode +
      '&taskId=' +
      taskId;
    return this.httpClient.get(_url);
  ...
  updateAdminStatus(empCode: string, taskId: string) {
    let _url =
      this._serverURL +
      '/SystemNotification/UpdateAdminNotification?employeeCode=' +
      empCode +
      '&taskId=' +
      taskId;
    return this.httpClient.get(_url);
  ...
  profileApproval(empId: number, reqtype: string, reason: string) {
    let _url =
      this._employeeServerURL + servicePath.API.PAssociate.profileApproval;
    let details = {
      EmpId: empId,
      Status: reqtype,
      Remarks: reason,
