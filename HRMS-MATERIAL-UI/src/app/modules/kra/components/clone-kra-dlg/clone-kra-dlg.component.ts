...
import { KraDlgData } from "../../../kra/models/kra-dlg-data";
import { FormControl } from "@angular/forms";
import { FormBuilder, FormGroup } from "@angular/forms";
import { SelectionModel } from "@angular/cdk/collections";
import { AddKRAdlgComponent } from "../../../kra/components/add-kradlg/add-kradlg.component";
...
@Component({
  selector: 'app-clone-kra-dlg',
  templateUrl: './clone-kra-dlg.component.html',
  styleUrls: ['./clone-kra-dlg.component.scss']
})
export class CloneKraDlgComponent {
 ...
 list = [];
  ...
  constructor( public dialog: MatDialog,
    ...
    public dialogRef: MatDialogRef<CloneKraDlgComponent>,
    @Inject(MAT_DIALOG_DATA) public data: CloneKraDlgComponent) {}
...
ngOnInit() {
    this.cloneKRAForm = this.fb.group({
  ...
  getApplicableGradeRoleTypesByGrade() {
    ...
    //     this.departmentId,
    //     null,
    //     this.cloneKRAForm.value.gradeId
    //   )
    //   .subscribe(
    //     (res: any[]) => {
    //       if (res) {         
    ...
    //           this.cloneKRAForm.get("roleTypeId").setValue(this.roleTypeId);
    //           this.showDefineKRAs("");
    //         }
    //       } 
    //     },
  ...
  getFinancialYears(): void {
    this._masterDataService.GetFinancialYears().subscribe(
      (yearsdata: any[]) => {
        ...
        this.cloneKRAForm.controls["financialYearId"].enable();
      },
      error => {
  ...
  getDepartments(): void {
    this._masterDataService.GetDepartmentsMasterData().subscribe(
      ...
      this.cloneKRAForm.controls["departmentId"].enable();
      },
      error => {
  ...
  onchange(event) {
    ...
    this.cloneKRAForm.get("departmentId").setValue(null);
    this.cloneKRAForm.get("gradeId").setValue(null);
    this.cloneKRAForm.get("roleTypeId").setValue(null);   
    this.dataSource = null;     
  }
  ...
  getApplicableGradeRoleTypes() {   
    ...
    //         this.dataSource = null;          
    //         if (this.gradeId > 0) {
    //           this.cloneKRAForm.get("gradeId").setValue(this.gradeId);
...
// }
  ...
  Import(){     
    ...
    if(length==0)
    {
       this._snackBar.open("Please select a records to copy the KRA's.", "", {
            duration: 1000,
            horizontalPosition: "right",
            verticalPosition: "top"
          });
    ...
    this._definekraService
      .getKRADefinitionsById(
        ...
        this.departmentId,
        this.cloneKRAForm.value.gradeId,
        this.cloneKRAForm.value.roleTypeId
      )
      .subscribe(
        (res: DefineKRAData[]) => {
          if (res) {
