...
@Directive({
  selector: '[appPracticeAreaValidations]'
})
export class PracticeAreaValidationsDirective {
  ...
  @HostListener('keyup', ['$event']) onKeyPress(event: any) {
    ...
    if (!this.regex.test(event.key) && valStr.length > 1) {
      ...
      // }
      this._snackBar.open(
        'Begin with Alphanumerics. Special Characters except ( and ) are not allowed',
        'x',
        {
          duration: 3000,
...
@NgModule({
  declarations: [PracticeAreaValidationsDirective],
  exports: [PracticeAreaValidationsDirective]
})
export class PracticeAreaValidationsDirectiveModule { }
