...
@Directive({
  selector: '[appGradesValidations]'
})
export class GradesValidationsDirective {
  //Regex for following alphanumerics
  private regex: RegExp = /^(?:[A-Za-z0-9 -]+)([A-Za-z0-9]*)$/;
  ...
  @HostListener('keyup', ['$event']) onKeyPress(event: any) {
    ...
    if (!this.regex.test(event.key) && valStr.length > 1) {
      ...
      if (event.shiftKey || valStr.charCodeAt(0) == 45) {
        if ((valStr.charCodeAt(0) < 47 || valStr.charCodeAt(0) > 122) || (valStr.charCodeAt(0) < 65 || valStr.charCodeAt(0) > 90) || valStr.charCodeAt(0) == 45) {
          this._snackBar.open(
            'Begin with Alphanumerics. Special Characters except - are not allowed',
            'x',
            {
              duration: 3000,
              horizontalPosition: 'right',
...
@NgModule({
  declarations: [GradesValidationsDirective],
  exports: [GradesValidationsDirective]
})

export class GradesValidationsDirectiveModule { }
